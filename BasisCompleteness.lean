...
/-- The inductive semiring algebra generated by the four PLEX basis matrices (I, R, A, S).
    We omit negative coefficients since combinators cannot represent subtraction/negation. -/
...
/-- The adjacency matrix A1 is representable by some term. -/
...
Any matrix generated in the semiring basis algebra of $\{I, R, A, S\}$ can be expressed
...
theorem basis_expressive_completeness (M : Matrix4) (h : BasisAlgebra M) :
  ...
  | add M1 M2 h1 h2 ih1 ih2 =>
      let ⟨t1, ht1⟩ := ih1
      let ⟨t2, ht2⟩ := ih2
      let ⟨t3, ht3⟩ := term_matrix_add t1 t2
      exact ⟨t3, by rw [ht3, ht1, ht2]⟩
